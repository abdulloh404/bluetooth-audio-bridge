...
Pair and connect Bluetooth devices through Ubuntu. While forwarding is enabled and PipeWire is connected, the controller also opens the A2DP Source profile on already-connected, paired source devices that have no incoming A2DP transport. This handles phones that Ubuntu connects over Bluetooth LE without opening the Bluetooth Classic audio connection. It waits briefly for phone-initiated connections and leaves existing A2DP transports alone, including idle transports when playback is paused. Failed requests are retried after the configured delay and reported through status and the service journal. Device paths are discovered from BlueZ; no fixed phone address is required. The controller does not pair devices, connect disconnected devices, or change headphone profiles. PipeWire reconnection is retried without restarting any audio service.
...
The `[bluetooth]` section controls `dbus_timeout_seconds`, `a2dp_connect_delay_seconds`, and `a2dp_retry_delay_seconds`. The controller and the `devices` command use these settings from the active config file; `--config` selects another file. All values are integer seconds. D-Bus timeout and retry delay must be greater than zero; the initial A2DP delay may be zero. Missing fields in older config files use defaults bundled from `config/default.toml`, which is also the installation template. Existing config files are preserved during installation. Edit the active config and restart the controller yourself to apply timing changes without rebuilding; `devices` reads the file on each invocation and can use project defaults before initialization. Standard A2DP UUIDs remain fixed in code.
