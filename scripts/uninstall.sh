...
config_dir="$config_home/bluetooth-audio-bridge"
unit_dir="$config_home/systemd/user"
data_dir="${XDG_DATA_HOME:-$HOME/.local/share}/bluetooth-audio-bridge"
state_dir="${XDG_STATE_HOME:-$HOME/.local/state}/bluetooth-audio-bridge"
cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/bluetooth-audio-bridge"
runtime_dir="${XDG_RUNTIME_DIR:?Run this uninstaller from your desktop user session}/bluetooth-audio-bridge"
...
if [ -e "$data_dir/install-marker" ] || [ -L "$data_dir/install-marker" ]; then
    if [ -L "$data_dir/install-marker" ] || [ ! -f "$data_dir/install-marker" ] || [ "$(cat "$data_dir/install-marker")" != 'BLUETOOTH_AUDIO_BRIDGE_INSTALL=1' ]; then
...
for item in "$unit_dir/bluetooth-audio-bridge.service" "$bin_dir/bluetooth-audio-bridge" "$bin_dir/bluetooth-audio-bridged" "$bin_dir/bluetooth-audio-bridge-phone-policy"; do
...
for policy in "$config_home/wireplumber/bluetooth.lua.d/90-bluetooth-audio-bridge-phone.lua" "$config_home/wireplumber/wireplumber.conf.d/90-bluetooth-audio-bridge-phone.conf"; do
    ...
    if [ -f "$policy" ]; then
        case "$(head -n 1 "$policy")" in
            '-- BLUETOOTH_AUDIO_BRIDGE_POLICY=1'|'# BLUETOOTH_AUDIO_BRIDGE_POLICY=1') ;;
            *) printf '%s\n' "Policy is not owned by Bluetooth Audio Bridge: $policy" >&2; exit 1 ;;
        esac
    fi
done
...
if systemctl --user is-active --quiet bluetooth-audio-bridge.service; then
    printf '%s\n' 'The user service is active. Stop and disable it yourself before uninstalling.' >&2
    exit 1
fi
if systemctl --user is-enabled --quiet bluetooth-audio-bridge.service; then
...
if ! flock -n 9; then
    printf '%s\n' 'Bluetooth Audio Bridge is running or another command is updating it. Close it yourself before uninstalling.' >&2
    exit 1
fi
...
if systemctl --user is-failed --quiet bluetooth-audio-bridge.service; then
    systemctl --user reset-failed bluetooth-audio-bridge.service
fi
...
for temporary in "$config_home/wireplumber/bluetooth.lua.d"/.bluetooth-audio-bridge-* "$config_home/wireplumber/wireplumber.conf.d"/.bluetooth-audio-bridge-*; do
...
rm -f -- "$bin_dir/bluetooth-audio-bridge" "$bin_dir/bluetooth-audio-bridged" "$bin_dir/bluetooth-audio-bridge-phone-policy" "$unit_dir/bluetooth-audio-bridge.service"
